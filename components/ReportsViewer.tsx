...
interface ReportsViewerProps {
    users?: User[]; // Para buscar el teléfono del admin
}

const ReportsViewer: React.FC<ReportsViewerProps> = ({ users = [] }) => {
    ...
    const handleSendToAdmin = async (report: Report) => {
        ...
        // Logic: Download file so user can attach it, then open WhatsApp
        if (confirm("Se descargará el PDF en tu dispositivo. Por favor, adjúntalo en el chat de WhatsApp que se abrirá a continuación.")) {
            // 1. Download
            handleDownload(report);
            
            // 2. Open WhatsApp
            const message = `Hola, adjunto el reporte: ${report.fileName}`;
            const url = `https://wa.me/${phone}?text=${encodeURIComponent(message)}`;
            window.open(url, '_blank');

            // 3. Update Status
            await updateReportStatus(report.id, { sent: true });
             setReports(prev => prev.map(r => r.id === report.id ? { ...r, status: { ...r.status, downloaded: true, sent: true } } : r));
        }
    };
    ...
    const summaryData = React.useMemo(() => {
        const stats: Record<string, { total: number, downloaded: number, sent: number }> = {};
        
        reports.forEach(r => {
            if (!stats[r.program]) {
                stats[r.program] = { total: 0, downloaded: 0, sent: 0 };
            }
            stats[r.program].total++;
            if (r.status?.downloaded) stats[r.program].downloaded++;
            if (r.status?.sent) stats[r.program].sent++;
        });
                    ...
                    {reports.map((report) => (
                        <div key={report.id} className="bg-white dark:bg-white/5 p-4 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm flex flex-col gap-3 group hover:border-primary/50 transition-colors relative overflow-hidden">
                            ...
                            <div className="absolute top-0 right-0 p-2 flex gap-1">
                                {report.status?.sent && <span title="Enviado al Admin" className="size-2 rounded-full bg-green-500"></span>}
                            ...
                            <div className="flex gap-2 justify-end border-t border-gray-100 dark:border-white/5 pt-3">
                                 <button 
                                    onClick={() => handleSendToAdmin(report)}
                                    className="flex-1 bg-green-500/10 text-green-700 dark:text-green-400 text-[10px] font-bold py-2 rounded flex items-center justify-center gap-1 hover:bg-green-500 hover:text-white transition-colors"
                                >
                                    <span className="material-symbols-outlined text-sm">send</span> WhatsApp Admin
                                </button>
                                <button 
                            ...
                            <table className="w-full text-xs">
                                <thead>
                                    <tr className="text-gray-500 text-left border-b border-gray-100 dark:border-white/5">
                                        ...
                                        <th className="py-2 font-bold text-center">Env.</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {summaryData.map(row => (
                                        <tr key={row.program} className="border-b border-gray-50 dark:border-white/5 last:border-0">
                                            ...
                                            <td className="py-2 text-center text-green-500 font-bold">{row.sent}</td>
                                        </tr>
                                    ))}
