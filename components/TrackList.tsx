...
const TrackList: React.FC<TrackListProps> = ({ 
  ...
  const displayItems = useMemo(() => {
      ...
      // Si no hay búsqueda, mostrar carpetas y archivos en el nivel actual
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="sticky top-0 z-10 bg-white border-b">
        {!isSelectionView && (
            <div className="bg-azul-header flex items-center overflow-x-auto no-scrollbar">
                ...
                {isAdmin && (
                    <button onClick={() => setShowAddRootModal(true)} className="px-4 py-4 text-miel flex-none" title="Nueva Carpeta">
        ...
        <div className="p-3 space-y-2">
            <div className="flex items-center gap-2">
                 <div className="relative flex-1">
                    ...
                    <input className="w-full pl-8 pr-3 py-2 rounded-lg border bg-gray-50 text-xs outline-none focus:border-primary" placeholder="Buscar título o intérprete..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} />
                 </div>
                 {!isSelectionView && isAdmin && (
                    <label className="bg-primary text-white size-8 flex items-center justify-center rounded-lg cursor-pointer flex-none">
            ...
            {!isSelectionView && searchQuery.trim() && (
                <div className="flex items-center gap-2">
                    <button onClick={() => setIsGlobalSearch(false)} className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-lg border ${!isGlobalSearch ? 'bg-miel text-white border-miel' : 'text-gray-400 border-gray-200'}`}>En esta carpeta</button>
                    <button onClick={() => setIsGlobalSearch(true)} className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-lg border ${isGlobalSearch ? 'bg-miel text-white border-miel' : 'text-gray-400 border-gray-200'}`}>Búsqueda Global</button>
                </div>
            )}

            {currentPath && !isSelectionView && !searchQuery.trim() && (
                <button onClick={() => setCurrentPath(currentPath.includes('/') ? currentPath.substring(0, currentPath.lastIndexOf('/')) : '')} className="flex items-center gap-1 text-[10px] font-bold text-azul-header uppercase">
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-gray-50 pb-20">
        {displayItems.map(item => (
          <div key={item.key} className="flex items-center gap-4 px-4 py-3 hover:bg-gray-50 transition-colors">
            ...
            <div className="flex-1 min-w-0" onClick={() => item.type === 'folder' ? setCurrentPath(item.fullPath) : onSelectTrack(item.data)}>
                ...
                <p className="text-[10px] text-gray-400 truncate mt-0.5">{item.type === 'track' ? item.data.metadata.performer : 'Directorio'}</p>
            </div>
            {item.type === 'track' && onToggleSelection && (
                <button onClick={() => onToggleSelection(item.data)} className={`size-8 rounded-full flex items-center justify-center border transition-all ${selectedTrackIds?.has(item.data.id) ? 'bg-primary border-primary text-white shadow-md' : 'border-gray-200 text-gray-300'}`}>
        ...
        {displayItems.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-2">
                ...
                <p className="text-xs font-medium">No hay resultados para mostrar.</p>
            </div>
        )}
      </div>
